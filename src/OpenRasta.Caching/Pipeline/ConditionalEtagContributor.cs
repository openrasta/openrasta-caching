...
namespace OpenRasta.Caching.Pipeline
{
    public class ConditionalEtagContributor : ConditionalContributor, IPipelineContributor
    {
        ...
        PipelineContinuation ProcessPostConditional(ICommunicationContext context)
        {
            ...
            ProcessIf(context, HttpHeaders.IF_NONE_MATCH, ProcessIfNoneMatch);
            ProcessIf(context, HttpHeaders.IF_MATCH, ProcessIfMatch);
            // we ignore if-none-match for now. Yay!
            return PipelineContinuation.Continue;
        }

        void ProcessIf(ICommunicationContext context, string requestHeader, Action<ICommunicationContext, string,IEnumerable<ETagValidator>> process)
        {
            context.Request.Header(requestHeader, requestIf =>
            context.Response.Header(HttpHeaders.ETAG, entityTag =>
                process(context, entityTag, ParseEtags(requestIf))));
        }

        static void ProcessIfNoneMatch(ICommunicationContext context, string entityTag, IEnumerable<ETagValidator> validators)
        {
