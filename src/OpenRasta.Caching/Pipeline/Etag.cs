...
namespace OpenRasta.Caching.Pipeline
{
    ...
    {
        public static string StrongEtag(string partialEtag, MediaType mediaType = null, Encoding charset = null, CultureInfo language = null, string encoding = null)
        {
            // TODO: Once the design has settled, make the etag generation strategy swappable
            return string.Format(@"""{0}:{1}:{2}:{3}:{4}""", mediaType, charset,language,encoding, SanitizeEtag(partialEtag));
        }
