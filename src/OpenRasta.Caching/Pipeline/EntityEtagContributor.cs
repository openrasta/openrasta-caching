...
namespace OpenRasta.Caching.Pipeline
{
    public class EntityEtagContributor : IPipelineContributor
    {
        ...
        PipelineContinuation PostExecution(ICommunicationContext context)
        {
            ...
            var matchingRegistration = _config.ResourceRegistrations.FindAll(context.OperationResult.ResponseResource.GetType());

            Func<object, string> nullReader = resource => null;
            var reader = matchingRegistration.Select(_ => ResourceExtensions.GetEtagMapper(_))
            ...
            context.Response.Headers[HttpHeaders.ETAG] = GenerateEtag(context, partialEtag);
        ...
        bool ShouldSendETag(ICommunicationContext context)
        {
            return context.OperationResult.StatusCode == 200 && 
                   ...
                   !context.Response.Headers.ContainsKey(HttpHeaders.ETAG);
        }
    }
}